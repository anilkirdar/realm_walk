...
class LocalManager {
  ...
  static Future<void> preferencesInit() async {
    ...
    ///this call is required to make sure previous data is deleted
    ///however it will work once, so everytime name should be changed

    if (instance.getBoolValue(LocalManagerKeys.isFirstRun5) ?? true) {
      instance.clearAll();
      instance.setBoolValue(LocalManagerKeys.isFirstRun5, false);
    }
    return;
  }
  ...
  Future<void> dangeriousSetBoolValue__(String key, bool value) async {
  ...
  bool? dangeriousGetBoolValue__(String key) => _preferences?.getBool(key);
}
